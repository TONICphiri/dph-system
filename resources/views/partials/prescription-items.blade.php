...
<div x-data="repeater(@js($rows), { medicine_id: '', medicine_name: '', dosage: '', frequency: '', duration_days: '', quantity: '', instructions: '' })">
    <template x-for="(row, index) in rows" :key="index">
        <div class="grid gap-3 border-b border-line px-5 py-4 {{ $compact ? '' : 'sm:grid-cols-2 lg:grid-cols-6' }}">
            ...
            <div><label class="label" :for="'item_dosage_' + index">Dosage</label><input class="input" :id="'item_dosage_' + index" :name="'items[' + index + '][dosage]'" x-model="row.dosage" placeholder="For example, 2 tablets"></div>
            <div>
